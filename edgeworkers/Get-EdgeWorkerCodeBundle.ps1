function Get-EdgeWorkerCodeBundle
{
    ...
    if($OutputFile){
        $BundleFileName = $OutputFile
    }
    else{
        if($Name){
            $BundleFileName = "$Name-$Version.tgz"
        }
        else{
            $BundleFileName = "$EdgeWorkerID-$Version.tgz"
        }
    }

    try {
        $Result = Invoke-AkamaiRestMethod -Method GET -Path $Path -OutputFile $BundleFileName -AdditionalHeaders $AdditionalHeaders -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey
        return $Result
    }
    catch {
        throw $_
