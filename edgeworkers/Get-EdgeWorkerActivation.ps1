function Get-EdgeWorkerActivation
{
    ...
    if($ActivationID.ToLower() -eq "latest"){
        try{
            $Versions = List-EdgeWorkerActivations -EdgeWorkerID $EdgeWorkerID -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey
            $ActivationID = $Versions[0].activationId
        }
        catch{
            throw $_.Exception
        }
