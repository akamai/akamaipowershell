function Download-SharedCloudletPolicyVersion
{
    Param(
    ...
    if($Version -eq 'latest'){
        $Version = (List-v3CloudletPolicyVersions -PolicyID $PolicyID -Pagesize 1 -EdgeRCFile $EdgeRCFile -Section $Section -AccountSwitchKey $AccountSwitchKey).Version
        Write-Debug "Found latest version = $Version"
    }

    $Path = "/cloudlets/v3/policies/$PolicyID/versions/$Version/download?accountSwitchKey=$AccountSwitchKey"
