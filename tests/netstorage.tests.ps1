...
Describe 'Safe Netstorage Tests' {
    ...
    BeforeAll {
        ...
        $SafeEdgeRCFile = $env:PesterSafeEdgeRCFile
        $Section = 'default'
        $TestStorageGroupID = 1101059
    ...
    it 'List-NSStorageGroups returns the correct data' {
        $Groups = List-NSStorageGroups -EdgeRCFile $EdgeRCFile -Section $Section
        $Groups[0].storageGroupId | Should -Not -BeNullOrEmpty
    }

    ### Get-NSStorageGroup
    it 'Get-NSStorageGroup returns a group' {
        $Group = Get-NSStorageGroup -StorageGroupID $TestStorageGroupID -EdgeRCFile $EdgeRCFile -Section $Section
        $Group | Should -Not -BeNullOrEmpty
    }

    ### List-NSUploadAccounts
    it 'List-NSUploadAccounts returns the correct data' {
        $UploadAccounts = List-NSUploadAccounts -EdgeRCFile $EdgeRCFile -Section $Section
        $UploadAccounts[0].uploadAccountId | Should -Not -BeNullOrEmpty
    }

    ### Get-NSUploadAccount
    it 'Get-NSUploadAccount returns an account' {
        $PD.UploadAccount = Get-NSUploadAccount -UploadAccountID $TestUploadAccountID -EdgeRCFile $EdgeRCFile -Section $Section
        $PD.UploadAccount | Should -Not -BeNullOrEmpty
    }

    ### List-NSZones
    it 'List-NSZones returns a list' {
        $Zones = List-NSZones -EdgeRCFile $EdgeRCFile -Section $Section
        $Zones.zones.count | Should -Not -BeNullOrEmpty
    }

    ### Set-NSUploadAccount by pipeline
    it 'Set-NSUploadAccount by pipeline updates details' {
        $SetAccountByPipeline = ( $PD.UploadAccount | Set-NSUploadAccount -UploadAccountID $TestUploadAccountID -EdgeRCFile $EdgeRCFile -Section $Section )
        $SetAccountByPipeline.uploadAccountId | Should -Be $TestUploadAccountID
    }

    ### Set-NSUploadAccount by param
    it 'Set-NSUploadAccount by param updates details' {
        $SetAccountByParam = Set-NSUploadAccount -UploadAccountID $TestUploadAccountID -UploadAccount $PD.UploadAccount -EdgeRCFile $EdgeRCFile -Section $Section
        $SetAccountByParam.uploadAccountId | Should -Be $TestUploadAccountID
    }

    ### Set-NSUploadAccount by json
    it 'Set-NSUploadAccount by json updates details' {
        $SetAccountByBody = Set-NSUploadAccount -UploadAccountID $TestUploadAccountID -Body (ConvertTo-Json $PD.UploadAccount -Depth 10) -EdgeRCFile $EdgeRCFile -Section $Section
...
Describe 'Unsafe Netstorage Tests' {
    BeforeAll {
        ...
        $SafeEdgeRCFile = $env:PesterSafeEdgeRCFile
        $Section = 'default'
        $TestStorageGroupID = 123456
        $TestGroup = Get-NSStorageGroup -StorageGroupID $TestStorageGroupID -EdgeRCFile $SafeEdgeRCFile -Section $Section
        $TestUploadAccount = Get-NSUploadAccount -UploadAccountID MyUploadAccount -EdgeRCFile $SafeEdgeRCFile -Section $Section
    }

    ### New-NSStorageGroup by pipeline
    it 'New-NSStorageGroup creates a group' {
        $NewGroupByPipeline = ($TestGroup | New-NSStorageGroup -EdgeRCFile $SafeEdgeRCFile -Section $Section)
        $NewGroupByPipeline | Should -Not -BeNullOrEmpty
    }

    ### New-NSStorageGroup by param
    it 'New-NSStorageGroup creates a group' {
        $NewGroupByParam = New-NSStorageGroup -StorageGroup $TestGroup -EdgeRCFile $SafeEdgeRCFile -Section $Section
        $NewGroupByParam | Should -Not -BeNullOrEmpty
    }

    ### New-NSStorageGroup by body
    it 'New-NSStorageGroup creates a group' {
        $NewGroupByBody = New-NSStorageGroup -Body (ConvertTo-Json $TestGroup -Depth 100) -EdgeRCFile $SafeEdgeRCFile -Section $Section
        $NewGroupByBody | Should -Not -BeNullOrEmpty
    }

    ### New-NSUploadAccount by pipeline
    it 'New-NSUploadAccount creates successfully' {
        $NewAccountByPipeline = ( $TestUploadAccount | New-NSUploadAccount -EdgeRCFile $SafeEdgeRCFile -Section $Section )
        $NewAccountByPipeline | Should -Not -BeNullOrEmpty
    }

    ### New-NSUploadAccount by param
    it 'New-NSUploadAccount creates successfully' {
        $NewAccountByParam = New-NSUploadAccount -UploadAccount $TestUploadAccount -EdgeRCFile $SafeEdgeRCFile -Section $Section
        $NewAccountByParam | Should -Not -BeNullOrEmpty
    }

    ### New-NSUploadAccount by json
    it 'New-NSUploadAccount creates successfully' {
        $NewAccountByBody = New-NSUploadAccount -Body (ConvertTo-Json $TestUploadAccount -Depth 10) -EdgeRCFile $SafeEdgeRCFile -Section $Section
        $NewAccountByBody | Should -Not -BeNullOrEmpty
    }

    ### Set-NSStorageGroup by pipeline
    it 'Set-NSStorageGroup updates details' {
        $SetGroupByPipeline = ( $TestGroup | Set-NSStorageGroup -StorageGroupID $TestStorageGroupID -EdgeRCFile $SafeEdgeRCFile -Section $Section )
        $SetGroupByPipeline.storageGroupId | Should -Not -BeNullOrEmpty
    }

    ### Set-NSStorageGroup by param
    it 'Set-NSStorageGroup updates details' {
        $SetGroupByParam = Set-NSStorageGroup -StorageGroupID $TestStorageGroupID -StorageGroup $TestGroup -EdgeRCFile $SafeEdgeRCFile -Section $Section
        $SetGroupByParam.storageGroupId | Should -Not -BeNullOrEmpty
    }

    ### Set-NSStorageGroup by json
    it 'Set-NSStorageGroup updates details' {
        $SetGroupByBody = Set-NSStorageGroup -StorageGroupID $TestStorageGroupID -Body (ConvertTo-Json $TestGroup -Depth 10) -EdgeRCFile $SafeEdgeRCFile -Section $Section
